...
def manual_test():
    ...
    if result != 0:
        ...
        after = "\n".join(after)
        # build diff
        task = {"diff": {"file": "utils/add.py", "before": before, "after": after}}
        patch = TaskExecutor("utils").build_patch(task)
        print("---Proposed Diff---")
        print(patch)
